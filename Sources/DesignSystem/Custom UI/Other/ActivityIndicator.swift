...
//  diffibleData
//
//  Created by Arman Davidoff on 14.12.2020.
...
public class CustomActivityIndicator: UIView, CAAnimationDelegate {
    ...
    @IBInspectable public var strokeColor: UIColor = UIColor.mainApp() {
        didSet{
    ...
    public var hideAfterTime: TimeInterval = CustomActivityIndicator.dhHidesWhenCompletedDelay
    ...
    public func startLoading() {
        ...
        progressLayer.add(animation, forKey: CustomActivityIndicator.dhRingRotationAnimationKey)
        ...
        progressLayer.add(animations, forKey: CustomActivityIndicator.dhRingRotationAnimationKey)
    }
    ...
    public func completeLoading(success: Bool, completion: Block? = nil) {
        ...
        var phase1Duration = 0.7 * CustomActivityIndicator.dhCompletionAnimationDuration
        var phase2Duration = 0.3 * CustomActivityIndicator.dhCompletionAnimationDuration
        ...
        if !success {
            ...
            phase1Duration = 0.5 * CustomActivityIndicator.dhCompletionAnimationDuration
            phase2Duration = 0.2 * CustomActivityIndicator.dhCompletionAnimationDuration
            phase3Duration = 0.3 * CustomActivityIndicator.dhCompletionAnimationDuration
        }
    ...
    public func animationDidStop(_: CAAnimation, finished flag: Bool) {
        if hidesWhenCompleted {
            Timer.scheduledTimer(timeInterval: CustomActivityIndicator.dhHidesWhenCompletedDelay, target: self, selector: #selector(hiddenLoadingView), userInfo: nil, repeats: false)
        } else {
            status = .Completion
    ...
    @objc private func resetAnimations() {
        if status == .Loading {
            status = .Unknown
            progressLayer.removeAnimation(forKey: CustomActivityIndicator.dhRingRotationAnimationKey)
            progressLayer.removeAnimation(forKey: CustomActivityIndicator.dhRingStorkeAnimationKey)
